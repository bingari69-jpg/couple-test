...
## 오목·알까기 배포 요청 (2026-09-14)

- 사용자가 두 게임 커밋·배포를 승인했다. 원격 main 일치 및 npm test 전체 통과(51페이지 × 9주소 검사 포함)를 확인했다.
- 서버 SQL은 사용자가 직접 실행한다. 두 마이그레이션을 한 트랜잭션으로 묶어 채팅에 전체 SQL을 제공했고, output/board-games-server.sql에도 저장했다. 실제 운영 적용/서버 검증 결과는 후속 기록으로 확인한다.
...
- `supabase/migrations/20260914_alkkagi_rooms.sql` 운영 적용과 앱 배포는 남아 있다. 커밋하지 않았다. 관리 화면 접근이 브라우저 주소 확인 실패로 중단된 상태를 이어받았으며, 로컬 체험을 운영 연결 완료로 설명하면 안 된다.
...
- **운영 SQL 적용·프런트 배포·커밋은 아직 하지 않았다.** `supabase/migrations/20260914_omok_rooms.sql`을 운영 프로젝트에 적용해야 친구전이 작동한다. 기존 단체방 익명 접속을 재사용하며 별도 신규 테이블/RPC만 추가한다. 서버 관리 화면 자동 조작은 도구가 Windows 브라우저의 현재 URL을 확인하지 못해 중단됐다. 이 상태를 운영 연결 완료로 설명하면 안 된다.
