...
## 2026-09-16 홈 놀기 목록에서 심리테스트 분리

- 사용자 요청으로 홈의 둘이놀기·혼자놀기 목록에는 게임만 남기고, 심리테스트 4종(`love-note`·`closeness`·`mirror`·`know-me`)은 상단 `심리` 메뉴(`/t/psychology/`)에서만 보여준다.
- `home-catalog.js` 항목에 `mind:true` 를 달고 `home.js` 목록과 검색용 정적 목록에서 걸러낸다. 목록 자체에는 남아 있어 관리센터에서 제목·공개 여부를 그대로 다룰 수 있고, 개별 주소·sitemap·공유도 그대로다.
- 홈 카드 수 38 → 34(혼자놀기 17). `test/seo.js` 가 '놀기 목록에는 없고 심리 메뉴에는 있는지'를 검사한다.
