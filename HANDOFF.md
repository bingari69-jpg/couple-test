...
- 홈 상단에 `처음이야? 10초면 알 수 있어!` 진입 카드를 표시하고 푸터에도 사용법 링크를 둔다.
- 27개 게임·테스트에 `게임 방법` 버튼, 한 줄 규칙, 3단계, 도움말을 공통으로 제공한다. 처음 방문한 게임은 설명창을 한 번 자동으로 연다.
- 카톡 초대 링크로 들어오면 `친구가 같이 하자고 보냈어요!` 안내를 별도로 표시한다.
- 반응속도·멈추기·고르기·누르기 게임은 실제 기록에 들어가지 않는 짧은 연습을 제공한다.
- 전용 연습: 넌센스는 예시 문제, 가위바위보는 패 고르기, 색깔 함정은 글자색, 화살표 함정은 빨간 테두리 반대 방향을 보여준다. 게임과 무관한 공통 빨강·파랑 연습은 제거했다.
