...
- 정정: 처음에는 `t/memory`가 모양이 다른 링크에서 예외를 낸다고 기록했으나, 확인 결과 실제 페이지 문제가 아니라 테스트 도구의 한계였다. `test/dom.js`는 `../../assets/` 스크립트만 인라인하고 같은 폴더의 `data.js`는 싣지 않아서 jsdom에서 `window.MEMORY`가 비어 있었고, 해시와 무관하게 빈 주소에서도 같은 예외가 난다. 실제 브라우저에서는 `data.js`가 실리고, 기억 게임은 링크의 문항 id를 목록과 대조해 없으면 홈으로 보낸다. 기억 게임을 jsdom으로 검사하려면 `test/dom.js`가 같은 폴더의 스크립트도 인라인하도록 보완해야 한다.
