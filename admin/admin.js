(function () {
  ...
  function normalize(input) {
    ...
    // 코드에만 추가되고 저장된 설정에는 아직 없는 게임은 목록 맨 위에 붙인다.
    // 홈(home-catalog.js의 appendUnknownLocal)과 같은 규칙으로, 새 게임이 관리센터와 홈에서 묻히지 않게 한다.
    // 서버가 '숨김'으로 둔 게임은 next.games에 이미 있으므로 여기서 되살아나지 않는다.
    const known = new Set(next.games.map(game => game.slug));
    const newGames = base.games.filter(game => !known.has(game.slug));
    if (newGames.length) {
      const firstOrder = next.games.reduce((min, game) => Math.min(min, Number(game.sortOrder) || 0), 1) - newGames.length;
      newGames.forEach((game, index) => {
        next.games.push(Object.assign({}, game, { featured: true, sortOrder: firstOrder + index }));
      });
    }
  ...
  function renderAds(){const list=$('adList');list.replaceChildren();$('adsEnabled').checked=!!config.ads.enabled;renderPublishedAdsState();config.ads.slots.forEach((ad,index)=>{
    ...
    const enabled=card.querySelector('[data-field="enabled"]');enabled.checked=!!ad.enabled;enabled.onchange=()=>{ad.enabled=enabled.checked;head();renderPublishedAdsState();changed();};
  ...
  function renderPublishedAdsState(){
    ...
    const live = (ads.slots || []).filter(slot => slot && slot.enabled).length;
    const on = !!ads.enabled && live > 0;
    el.textContent = on ? '지금 공개 중: 광고 보이는 중 · 자리 ' + live + '개'
      : !ads.enabled ? '지금 공개 중: 광고 꺼짐 — 위의 «전체 광고» 가 꺼져 있습니다.'
      : '지금 공개 중: 광고 꺼짐 — 전체 광고는 켜져 있지만, 자리마다 «이 자리 광고 켜기» 가 모두 꺼져 있습니다.';
