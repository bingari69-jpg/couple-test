...
(function () {
  ...
  Duel.setup = function (cfg) {
    ...
    if (cfg.shareChallenge !== false) {
      $("kakaoBtn").onclick = () => withName(() => kakaoShare({
    ...
    function renderResult(R) {
      ...
      if (isB) {
        ...
        $("sendResult").onclick = () => withName(() => { $("resLinkbox").textContent = rurl; copy(rurl, "결과 링크 복사됐어. 상대에게 보내"); });
        {
          const nm = n => n ? n + " " : "";
          const wn = o === "tie" ? null : (o === "win" ? me.n : them.n);
          const tail = o === "tie" ? " — 무승부" : (wn ? ` — ${wn} 승` : "");
          $("kakaoRes").onclick = () => withName(() => kakaoShare({
