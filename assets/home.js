...
const PLAY_HEROES=[
 {key:'mole',name:'두더지 잡기',eyebrow:'땅속에서 쏙!',accent:'두더지 잡기',rest:'한판 할래?',lead:'20초 동안 두더지 잡기.\n황금은 놓치지 마!',bubble:'쏙!',button:'두더지 잡으러 가기',href:'t/mole/',label:'고개를 내민 두더지',art:'atlas',sprite:[50,60],color:'#6d9f45'},
 {key:'rps',name:'가위바위보',eyebrow:'심심해? 뎀벼~',accent:'가위바위보로',rest:'한판 할래?',lead:'내가 먼저 낼게.\n카톡으로 보내면, 네가 받아줘!',bubble:'뎀벼!',button:'나 먼저 낼게',href:'t/rps/',label:'머리띠를 두른 장난스러운 토끼',art:'classic',color:'#ff6959'},
 {key:'hidden-picture',name:'숨은그림찾기',eyebrow:'그림 속에 꼭꼭',accent:'숨은그림찾기',rest:'같이 할래?',lead:'혼자는 세 장의 그림을.\n친구와는 30초 대결!',bubble:'찾았다!',button:'숨은 물건 찾기',href:'t/hidden-picture/?mode=online',label:'숲속 숨은그림찾기 장면',art:'hidden-picture',color:'#27967b'}
];
/* 놀이 홈과 편지 홈이 같은 게임을 가리켜야 한다.
   편지 홈에서는 "그럼 ○○ 한판" 제목과 버튼만 바꾼다(예전에는 제목은 가위바위보인데 버튼만 두더지로 바뀌었다). */
function pickHero(){
 ...
 return PLAY_HEROES[index];
}
function setPlayHero(){
 const hero=pickHero();
 $('rpsStart').href=hero.href;$('rpsStart').firstChild.textContent=hero.button+' ';
 if(!playing){
  const head=$('letterChallenge').querySelector('h2'),name=document.createElement('span');
  name.className='accent';name.textContent=hero.name;
  head.replaceChildren(document.createTextNode('편지가 쑥스러워?'),document.createElement('br'),document.createTextNode('그럼 '),name,document.createTextNode(' 한판'));
  return;
 }
 const play=$('play'),title=$('playTitle'),accent=document.createElement('span');
 ...
 $('playBubble').textContent=hero.bubble;
