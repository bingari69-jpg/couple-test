(function(){
 ...
 let view='library',occasion=occasionMap[params.get('occasion')]||'전체',chuseokGroup='전체',returnFromPreview='compose',preview=false,incoming=null,changingPaper=false,toastTimer,animationTimer,madeUrl='',sdkPromise;
 let activeReveal;
 function stopReveal(){if(activeReveal){activeReveal.finish();activeReveal=null;}}
 function startReveal(body,button){
  stopReveal();
  const element=$(body),skip=$(button);
  if(typeof window.revealLetter==='function')activeReveal=window.revealLetter(element,skip);
  else{skip.hidden=true;element.setAttribute('tabindex','-1');element.focus({preventScroll:true});}
 }
 const titles={library:'편지지 고르기',chuseok:'추석 감사편지',detail:'편지지 미리보기',compose:'마음 쓰기',send:'편지 보내기',reader:'도착한 편지',error:'편지 확인'};
 ...
 function cleanDraft(p){
   ...
   return {template:t.id,to:typeof p.to==='string'?p.to.slice(0,24):'',from:typeof p.from==='string'?p.from.slice(0,24):'',body:p.body,inline:I.clean(p.body,p.inline),font:Object.hasOwn(fonts,p.font)?p.font:t.font,size:Number.isFinite(p.size)?Math.max(16,Math.min(26,p.size)):t.size,occasion:['plain','chuseok','bday','day','year','wed','thanks','sorry','cheer'].includes(p.occasion)?p.occasion:'plain',number:Number.isInteger(p.number)?Math.max(1,Math.min(9999,p.number)):100,stickers:Array.isArray(p.stickers)?[...new Set(p.stickers.filter(id=>D.sticker(id)))].slice(0,3):[],seal:D.sticker(p.seal)?p.seal:'heart',color:COLORS.includes(p.color)?p.color:'',updated:Number.isFinite(p.updated)?p.updated:0};
 }
 ...
 $('deleteDraft').onclick=()=>requestNew(()=>{Object.assign(draft,{body:'',to:'',from:'',stickers:[],inline:[],seal:'heart',color:''});ownDraft=false;renderSaved();toast('이 기기의 초안을 지웠어요.');},true);
 $('quickWrite').onclick=startWriting;
 $('chuseokHero').hidden=!seasonal.active;
 ...
 function keepCaretVisible(){if(!window.requestAnimationFrame)return;requestAnimationFrame(()=>{
   if(view!=='compose'||document.activeElement!==$('letterEditor'))return;const selected=window.getSelection();if(!selected?.rangeCount)return;const range=selected.getRangeAt(0);if(!$('letterEditor').contains(range.endContainer)||!range.getBoundingClientRect)return;
   let rect=range.getBoundingClientRect();if(!rect.height&&range.endContainer.nodeType===1){const previous=range.endContainer.childNodes[range.endOffset-1];if(previous?.getBoundingClientRect)rect=previous.getBoundingClientRect();}
   if(!rect.height)return;const bottom=$('writingDock').getBoundingClientRect().top-18,top=(window.visualViewport?.offsetTop||0)+18;const delta=rect.bottom>bottom?rect.bottom-bottom:rect.top<top?rect.top-top:0;if(delta)window.scrollBy({top:delta,behavior:'instant'});
 });}
 function updateDock(){const v=window.visualViewport;const inset=v?Math.max(0,innerHeight-v.height-v.offsetTop):0;$('writingDock').style.setProperty('--keyboard-inset',inset+'px');$('compose').style.setProperty('--dock-height',($('writingDock').offsetHeight||70)+'px');keepCaretVisible();}
 if(window.visualViewport){visualViewport.addEventListener('resize',updateDock);visualViewport.addEventListener('scroll',updateDock);}window.addEventListener('resize',updateDock);
 ...
 function show(next,{historyMode='push',focus=true}={}){
   clearTimeout(animationTimer);stopReveal();view=next;closeDecorations();
   ...
   window.scrollTo(0,0);if(next==='compose'){resizeBody();updateDock();}if(focus){const h=$(next).querySelector('.screen-heading');if(h)h.focus({preventScroll:true});}
 }
 function go(next,options){
   if(next==='library')renderLibrary();if(next==='chuseok')renderChuseok();if(next==='detail')renderDetail();if(next==='compose')renderCompose();if(next==='send')renderSend();
   show(next,options);
 }
 ...
 function renderLibrary(){
   ...
   visible.forEach(t=>{
     ...
     const art=document.createElement('div');art.className='paper-art';paint(art,t);typography(art,t);art.setAttribute('aria-hidden','true');const copy=document.createElement('div');copy.className='mini-copy';const to=document.createElement('small');to.textContent=t.style==='한가위'?'HAPPY CHUSEOK':'TO. YOU';copy.append(to,document.createTextNode(t.style==='한가위'?'풍요로운 한가위,\n고마운 마음을 전해요.':t.style==='축하'?'네가 있어서,\n오늘이 더 특별해.':t.style==='캐릭터'?'밥은 잘 먹었어?\n그냥, 네 생각이 나서.':'별일은 없고,\n그냥 네 생각이 났어.'));art.append(copy);
 ...
 function renderChuseok(){
   $('chuseokFilters').replaceChildren();seasonal.groups.forEach(group=>{const b=document.createElement('button');b.className='chip';b.textContent=group;b.setAttribute('aria-pressed',String(chuseokGroup===group));b.onclick=()=>{chuseokGroup=group;renderChuseok();};$('chuseokFilters').append(b);});
   $('chuseokExampleGrid').replaceChildren();seasonal.examples.filter(x=>chuseokGroup==='전체'||x.group===chuseokGroup).forEach(x=>{
     const card=document.createElement('article');card.className='chuseok-example';const meta=document.createElement('div');meta.className='example-meta';const group=document.createElement('span');group.textContent=x.group;const tone=document.createElement('span');tone.textContent=x.tone;meta.append(group,tone);const title=document.createElement('h3');title.textContent=x.title;const preview=document.createElement('p');preview.className='example-preview';preview.textContent=x.body;const details=document.createElement('details');const summary=document.createElement('summary');summary.textContent='전체 문장 보기';const body=document.createElement('p');body.textContent=x.body;details.append(summary,body);const button=document.createElement('button');button.className='secondary';button.textContent='이 내용으로 편지 쓰기';button.onclick=()=>useChuseokExample(x);card.append(meta,title,preview,details,button);$('chuseokExampleGrid').append(card);
   });
 }
 function useChuseokExample(example){
   requestNew(()=>{const t=template(example.paper);Object.assign(draft,{template:t.id,to:'',from:'',body:example.body,font:t.font,size:t.size,occasion:'chuseok',number:1,stickers:[],inline:[],seal:'moon',color:''});ownDraft=true;changingPaper=false;go('compose');toast('감사 문구를 담았어요. 내 말로 편하게 고쳐보세요.');});
 }
 ...
 function renderCount(){
   ...
   $('letterBody').setAttribute('aria-invalid',String(over));$('letterEditor').setAttribute('aria-invalid',String(over));
 ...
 function syncDraft(){draft.to=$('recipient').value.slice(0,24);draft.from=$('sender').value.slice(0,24);draft.body=$('letterBody').value;draft.inline=I.clean(draft.body,draft.inline);editor.set(draft.body,draft.inline);renderCount();resizeBody();saveDraft();}
 ['recipient','sender','letterBody'].forEach(id=>$(id).addEventListener('input',syncDraft));
 function renderCompose(){paint($('composePaper'),template());typography($('composePaper'),draft);D.decorate($('composePaper'),draft.stickers);$('recipient').value=draft.to;$('sender').value=draft.from;$('letterBody').value=draft.body;editor.set(draft.body,draft.inline);selection={start:draft.body.length,end:draft.body.length};renderCount();$('fontChoice').value=draft.font;$('sizeChoice').value=draft.size;$('sizeValue').textContent=draft.size;$('occasionChoice').value=draft.occasion;$('anniversaryNumber').value=draft.number;renderOccasion();renderTools();saveDraft();}
 ...
 function renderSuggestions(){
   ...
   $('memoryQuestion').textContent=({chuseok:'올해 그 사람에게 가장 고마웠던 순간은 언제였나요?',bday:'지난 1년, 상대와 가장 많이 웃었던 날은 언제였나요?',thanks:'상대가 해 준 작은 일 중 아직 기억나는 것은?',sorry:'그때의 내 말 대신, 지금이라면 어떤 말을 건네고 싶나요?',cheer:'요즘 상대가 애쓰는 일과, 응원해 주고 싶은 점은?',day:'처음 만난 날, 가장 선명하게 떠오르는 장면은?',year:'함께한 시간 중 다시 돌아가 보고 싶은 하루는?',wed:'평범한 하루 속에서 고마움을 느낀 순간은?'})[draft.occasion]||'오늘 무엇을 하다가 상대가 떠올랐나요?';
   ...
   const themed=({chuseok:['풍요로운 한가위, 고마운 마음을 전하고 싶어요.','보름달처럼 환하고 넉넉한 명절 보내세요.','늘 곁에서 힘이 되어 주셔서 감사합니다.'],thanks:['그때는 미처 말 못 했는데, 정말 고마웠어.','네가 해 준 작은 일들이 오래 마음에 남아.'],sorry:['그날 내 말이 계속 마음에 걸렸어.','네 마음을 조금 더 잘 듣고 싶어.'],cheer:['요즘 네가 얼마나 애쓰는지 알아.','오늘은 내가 네 편이라는 말부터 하고 싶었어.']})[draft.occasion]||lines;
   $('suggestions').replaceChildren();themed.forEach(s=>{const b=document.createElement('button');b.textContent=s;b.onclick=()=>{const add=(draft.body?'\n\n':'')+s;if(D.segments(draft.body+add).length>MAX_LETTER){toast('이 문장을 더하면 450자를 넘어요. 글을 조금 줄여주세요.');return;}draft.body+=add;$('letterBody').value=draft.body;syncDraft();toast('쓴 글 뒤에 문장을 더했어요.');editor.select(draft.body.length);editor.focus();};$('suggestions').append(b);});
 }
 ...
 function decode(hash){
   ...
   if(p.v!=null&&(!Number.isInteger(p.v)||p.v<1||p.v>6))throw new Error('unsupported version');
 ...
 function read(p,isPreview){
   ...
   I.render($('readBody'),p.w!=null?p.w:window.LegacyLetter.linesOf(p).join('\n'),p.inl);$('readerEnvelopeStage').hidden=false;$('openedLetter').hidden=true;$('readerEnvelope').classList.remove('opening');
 }
 ...
 $('copyLetterText').onclick=async()=>{const text=incoming&&incoming.w!=null?I.plain(incoming.w,incoming.inl):$('readBody').textContent;try{await navigator.clipboard.writeText(text);toast('편지 본문을 복사했어요.');}catch(_){const range=document.createRange();range.selectNodeContents($('readBody'));const selected=window.getSelection();selected.removeAllRanges();selected.addRange(range);toast('편지 본문을 선택했어요. 길게 눌러 복사해 주세요.');}};
