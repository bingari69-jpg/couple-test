(function(){
 'use strict';
 const $=id=>document.getElementById(id),templates=window.LETTER_TEMPLATES,D=window.LetterDesign,I=window.LetterInline;
 ...
 function cleanDraft(p){
   ...
   return {template:t.id,to:typeof p.to==='string'?p.to.slice(0,24):'',from:typeof p.from==='string'?p.from.slice(0,24):'',body:p.body,inline:I.clean(p.body,p.inline),font:Object.hasOwn(fonts,p.font)?p.font:t.font,size:Number.isFinite(p.size)?Math.max(16,Math.min(26,p.size)):t.size,occasion:['plain','bday','day','year','wed','thanks','sorry','cheer'].includes(p.occasion)?p.occasion:'plain',number:Number.isInteger(p.number)?Math.max(1,Math.min(9999,p.number)):100,stickers:Array.isArray(p.stickers)?[...new Set(p.stickers.filter(id=>D.sticker(id)))].slice(0,3):[],seal:D.sticker(p.seal)?p.seal:'heart',color:COLORS.includes(p.color)?p.color:'',updated:Number.isFinite(p.updated)?p.updated:0};
 }
 ...
 function payload(){return {v:draft.inline.length?6:5,inl:draft.inline,w:draft.body,n:draft.to.trim(),f:draft.from.trim(),d:dateText(),tpl:draft.template,font:draft.font,size:draft.size,k:({day:0,year:1,wed:2,bday:3,plain:4,thanks:4,sorry:4,cheer:4})[draft.occasion],num:draft.number,rel:0,st:draft.stickers,seal:draft.seal,color:draft.color};}
