...
const {createDB}=require('../test/alkkagi-db.cjs');
async function serve(port=4188){
 const {db,rpc}=await createDB(),root=path.resolve(__dirname,'..');
 const server=http.createServer(async(req,res)=>{
  try{
   const url=new URL(req.url,'http://127.0.0.1');
   if(url.pathname==='/__alkkagi-rpc'&&req.method==='POST'){
    let raw='';for await(const chunk of req){raw+=chunk;if(raw.length>10000)throw Error('Too large');}
    const {fn,args}=JSON.parse(raw),user=Number(req.headers['x-alkkagi-player']);
   ...
   if(rel==='t/alkkagi/index.html'){
    body=body.toString().replace('<script src="online.js',`<script>
     const actorParam=new URLSearchParams(location.search).get('player');
     if(actorParam!==null)sessionStorage.setItem('alkkagi-preview-player',actorParam);
     const previewActor=Number(sessionStorage.getItem('alkkagi-preview-player')||0);
     window.GroupRoomService={ensureSession:async()=>({rpc(fn,args){return {abortSignal(signal){return fetch('/__alkkagi-rpc',{method:'POST',headers:{'Content-Type':'application/json','x-alkkagi-player':String(previewActor)},body:JSON.stringify({fn,args}),signal}).then(r=>r.json());}};}})};
    </script><script src="online.js`).replace('<main class="om-shell">','<main class="om-shell"><p style="padding:10px;background:#f7e4cc;font-size:12px;text-align:center">로컬 연결 체험 · 실제 카톡 초대 아님 · 화면 '+(url.searchParams.get('player')==='1'?'친구':'나')+'</p>');
   }
...
if(require.main===module)serve().then(s=>console.log('Local Alkkagi review: '+s.url+'/t/alkkagi/?mode=online&player=0')).catch(e=>{console.error(e);process.exitCode=1;});
