...
const results=[];
const close=starter=>{const xy=[[.22,.64],[.46,.64],[.70,.64],[.34,.82],[.62,.82]];return {stones:Array.from({length:10},(_,id)=>({id,color:id<5?1:2,x:+(id<5?xy[id%5][0]:1-xy[id%5][0]).toFixed(6),y:+(id<5?xy[id%5][1]:1-xy[id%5][1]).toFixed(6),alive:true})),turn:starter,shots:0,winner:0,draw:false};};
for(const [label,make,physicsVersion] of [['previous-line',legacy,1],['staggered-close',close,1],['staggered-spaced-old-force',R.create,1],['staggered-spaced-MAX',R.create,2]]){
 ...
 for(let seed=1;seed<=20;seed++)for(const starter of [1,2]){
  ...
  let s={...make(starter),physicsVersion};
...
fs.mkdirSync('output/alkkagi-v3',{recursive:true});fs.writeFileSync('output/alkkagi-v3/balance.json',JSON.stringify(results,null,2)+'\n');console.log(JSON.stringify(results,null,2));
