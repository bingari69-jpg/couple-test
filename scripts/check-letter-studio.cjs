...
(async()=>{
 ...
 assert.equal(await page.locator('#templateGrid > button').count(),26);
 ...
 await page.locator('#chuseokStart').click();assert.equal(await page.locator('#chuseokExampleGrid > article').count(),20);await page.screenshot({path:path.join(output,'chuseok-examples-mobile.png'),fullPage:true});await page.locator('#backToLetters').click();
 ...
 assert.deepEqual(errors,[]);console.log('Browser QA passed: 26 themes, 6 fonts, restoration, reader, image export, pagination, 320–1100 px.');
