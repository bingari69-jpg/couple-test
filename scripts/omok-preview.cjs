...
const {createDB}=require('../test/omok-db.cjs');
async function serve(port=4186){
 const {db,rpc}=await createDB(),root=path.resolve(__dirname,'..');
 const server=http.createServer(async(req,res)=>{
  try{
   const url=new URL(req.url,'http://127.0.0.1');
   if(url.pathname==='/__omok-rpc'&&req.method==='POST'){
    let raw='';for await(const chunk of req){raw+=chunk;if(raw.length>10000)throw Error('Too large');}
    const {fn,args}=JSON.parse(raw),user=Number(req.headers['x-omok-player']);
   ...
   if(rel==='t/omok/index.html'){
    body=body.toString().replace('<script src="online.js',`<script>
     const actorParam=new URLSearchParams(location.search).get('player');
     if(actorParam!==null)sessionStorage.setItem('omok-preview-player',actorParam);
     const previewActor=Number(sessionStorage.getItem('omok-preview-player')||0);
     window.GroupRoomService={ensureSession:async()=>({rpc(fn,args){return {abortSignal(signal){return fetch('/__omok-rpc',{method:'POST',headers:{'Content-Type':'application/json','x-omok-player':String(previewActor)},body:JSON.stringify({fn,args}),signal}).then(r=>r.json());}};}})};
...
if(require.main===module)serve().then(s=>console.log('Local Omok review: '+s.url+'/t/omok/?mode=online&player=0')).catch(e=>{console.error(e);process.exitCode=1;});
