(function(){
 ...
 function attach(hooks){
  ...
  function accept(s){
   ...
   hooks.apply({stones:s.board,shots:s.shots,turn:s.turn,winner:s.winner,draw:s.status==='finished'&&!s.winner,last:s.last,reason:s.reason,round:s.round,starter:s.starter,physicsVersion:s.physicsVersion||1},s.me);
  }
  ...
  function render(){
   ...
   $('roomNote').textContent=s.status==='waiting'?'내가 흑돌 다섯 개, 친구가 백돌 다섯 개예요. 아래 버튼으로 초대해주세요.':'이번 판은 '+(s.starter===2?'백돌':'흑돌')+'부터! '+(s.physicsVersion===2?'방향을 정하고, 힘 게이지를 멈춰 튕겨요.':'이 판은 기존 힘 규칙으로 진행돼요.');
