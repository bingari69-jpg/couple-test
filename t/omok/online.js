(function(){
 'use strict';
 const messages={OMOK_NOT_FOUND:'초대방을 찾지 못했어요. 초대 링크를 다시 확인해주세요.',OMOK_EXPIRED:'초대방의 24시간이 지났어요. 새 방에서 다시 만나요.',OMOK_FULL:'이미 두 사람이 두고 있어요. 참가했던 브라우저에서 열어주세요.',OMOK_NOT_MEMBER:'이 방의 참가자만 돌을 놓을 수 있어요.',OMOK_STALE:'상대의 새 소식이 도착했어요. 판을 확인하고 다시 골라주세요.',OMOK_NOT_TURN:'아직 상대 차례예요.',OMOK_OCCUPIED:'이미 돌이 놓인 자리예요. 다른 곳을 골라주세요.',OMOK_NOT_PLAYING:'아직 시작 전이거나 끝난 판이에요.',OMOK_INVALID_NAME:'이름을 1~12자로 적어주세요.',OMOK_RATE_LIMIT:'방을 많이 만들었어요. 잠시 후 다시 만들어주세요.',OMOK_AUTH_REQUIRED:'접속을 확인하지 못했어요. 다시 연결해주세요.',OMOK_NOT_FINISHED:'이번 판이 끝나면 한판 더 둘 수 있어요.'};
 function errorText(e){const msg=String(e&&e.message||e||'');const key=Object.keys(messages).find(k=>msg.includes(k));return key?messages[key]:/PGRST202|Could not find the function|does not exist/.test(msg)?'친구전 서버 연결을 준비 중이에요. 컴퓨터와 또는 한 화면에서 먼저 즐겨주세요.':'연결이 잠시 끊겼어요. 돌은 서버 확인 후에만 놓여요. 다시 연결해주세요.';}
 ...
 function attach(hooks){
  ...
  function accept(s){
   if(!s||!validCode(s.code)||!Array.isArray(s.board)||s.board.length!==225||s.board.some(n=>![0,1,2].includes(n))||!Array.isArray(s.moves)||s.moves.some(n=>!Number.isInteger(n)||n<0||n>=225)||!Number.isInteger(s.version)||![0,1,2].includes(s.me))throw Error('Invalid room response');
   ...
   hooks.apply({board:s.board.slice(),moves:s.moves.map((index,n)=>({index,color:n%2+1})),turn:s.turn,winner:s.winner,draw:s.reason==='draw',line:s.line||[]},s.me);
  }
  ...
  async function refresh(){
   ...
   try{const s=await rpc('get_omok_room',{p_code:code});if(ticket!==generation)return;accept(s);notice='';}
   catch(e){if(ticket!==generation)return;fresh=false;notice=errorText(e);terminal=/OMOK_EXPIRED|OMOK_NOT_FOUND/.test(String(e.message));}
  ...
  async function start(){if(working)return;const nickname=$('nickname').value.trim();if(!nickname||[...nickname].length>12){notice=messages.OMOK_INVALID_NAME;hooks.render();$('nickname').focus();return;}
   if(code){await action('join_omok_room',{p_code:code,p_name:nickname});return;}
   // Kept for retries until the user explicitly leaves this attempt.
   if(!request)request=crypto.randomUUID();await action('create_omok_room',{p_name:nickname,p_request:request});
  }
  async function open(value){if(!validCode(value)){notice='초대 링크가 올바르지 않아요. 친구에게 링크를 다시 받아주세요.';hooks.render();return;}
   ...
   try{const s=await rpc('get_omok_room',{p_code:code});if(ticket!==generation)return;accept(s);notice='';if(s.me)show();else if(s.status!=='waiting')notice=messages.OMOK_FULL;}
  ...
  function render(){
   ...
   $('start').textContent=online?(working?'방을 연결하고 있어요…':code?'초대받은 방 들어가기 →':'친구 초대방 만들기 →'):'오목 한판 시작하기 →';
   $('newInvite').hidden=!code;
   $('onlineIntro').textContent=snapshot&&!snapshot.me?snapshot.hostName+' 님이 오목 한판에 초대했어요.':'이름만 적으면 준비 끝. 카톡으로 친구 한 명을 초대해요.';
   ...
   $('roomTitle').textContent=s.status==='waiting'?'친구가 오면 한판 시작!':'멀리 있어도, 같은 바둑판';
   $('roomNote').textContent=s.status==='waiting'?'내가 흑돌, 친구가 백돌이에요. 아래 버튼으로 초대해주세요.':'상대가 둔 돌이 잠시 뒤 보여요. 내 차례에 한 알씩 놓아봐요.';
   ...
   $('turnStatus').textContent=!fresh?'연결을 확인할 때까지 잠시 기다려주세요.':working?'확인하고 있어요…':s.status==='waiting'?'초대 링크를 보내고 친구를 기다려요.':s.status==='finished'?(s.reason==='draw'?'무승부! 둘 다 잘 막았어요.':(s.winner===s.me?'내가 이겼어요!':'친구가 이겼어요!')):s.turn===s.me?'내 차례예요. 어디에 둘까요?':'친구 차례예요. 다음 수를 생각해봐요.';
   $('placement').hidden=s.status!=='playing';$('result').hidden=s.status!=='finished';
   if(s.status==='finished'){$('resultTitle').textContent=s.reason==='draw'?'빈틈없이 잘 막았어!':s.winner===s.me?'이번 한판은 내 승리!':'좋은 한판이었어!';$('resultNote').textContent=s.reason==='resign'?(s.winner===s.me?'친구가 이번 판을 양보했어요.':'이번 판은 친구에게 양보했어요.'):'다음에는 어떤 수를 둘까요?';if(s.rematchHost||s.rematchGuest)$('resultNote').textContent+=myVote?' 친구도 한판 더를 누르면 시작해요.':' 친구가 한판 더를 기다려요.';$('replay').textContent=myVote?'친구의 응답을 기다려요':'친구와 한판 더 →';$('replay').disabled=working||myVote||!fresh;}
  ...
  async function share(copyOnly){
   ...
   try{if(copyOnly)await copy();else await window.kakaoShare({title:'오목 한판 할래?',desc:'내가 흑돌, 네가 백돌. 다음 한 수는 너야!',url,btn:'오목방 들어가기',textOnly:true},copy);}
  ...
  $('resign').addEventListener('click',()=>$('resignDialog').showModal());$('resignCancel').addEventListener('click',()=>$('resignDialog').close());$('resignConfirm').addEventListener('click',()=>{$('resignDialog').close();if(snapshot)action('resign_omok_room',{p_code:code,p_version:snapshot.version});});
  ...
  return {render,start,leave,canPlay,init(){if(invite!==null){hooks.setMode('online');open(invite);}},move(index){if(canPlay())action('play_omok_move',{p_code:code,p_index:index,p_version:snapshot.version});},rematch(){if(snapshot&&!working)action('rematch_omok_room',{p_code:code,p_round:snapshot.round});}};
 }
 window.OmokOnline={attach,errorText};
})();
