...
function config(gameVisibility) {
  return {
    schemaVersion: 1,
    site: { name:'우리놀자', headingFont:'jua', bodyFont:'pretendard', fontScale:'1', primaryColor:'#123456', secondaryColor:'#654321', backgroundColor:'#fffaf0', homeHeroImage:'https://cdn.example.test/chuseok.webp', menu:[{id:'home',label:'처음',href:'./',enabled:true}] },
...
function runtimeDom(url, value) {
  ...
  return new JSDOM(`<!doctype html><html><head></head><body><main><a class="brand">같이놀자<b>♥</b></a><nav id="menu"></nav><div id="letterHome"><img class="hero-art" src="assets/art/envelope-hero.png"></div><span id="rpsPageTitle">10초</span></main><script>localStorage.setItem('gatchi_admin_preview',JSON.stringify(${payload}))</script><script>${runtime}</script></body></html>`, { url, runScripts:'dangerously', pretendToBeVisual:true });
}

(async function () {
  const adminHtml = read('admin/index.html');
  ['loginForm','adminApp','gameList','menuEditor','designForm','homeHeroImage','homeHeroImageFile','resetHomeHeroImage','adList','versionList','publishDialog'].forEach(id => assert(adminHtml.includes(`id="${id}"`), `관리 화면 #${id} 누락`));
  ...
  /* 저장된 설정에 없는 새 로컬 게임은 관리 목록 맨 위에 붙인다 — normalize 만 떼어 내 실행한다 */
  {
    ...
    assert.deepStrictEqual(merged.slice().sort((a,b)=>a.sortOrder-b.sortOrder).map(g => [g.slug, g.title, g.visibility, g.sortOrder, g.featured]), [['pairs','짝 맞추기','listed',0,true], ['ten','열 초 도전','hidden',3,false]], '코드에만 있는 새 게임이 목록 맨 위에 붙어야 함');
  ...
  const unsafeHomeConfig=config();unsafeHomeConfig.site.homeHeroImage='javascript:alert(1)';const unsafeHome=runtimeDom('https://example.test/couple-test/?admin_preview=1',unsafeHomeConfig);await new Promise(resolve=>setTimeout(resolve,20));assert.equal(unsafeHome.window.document.querySelector('#letterHome .hero-art').getAttribute('src'),'assets/art/envelope-hero.png');unsafeHome.window.close();
