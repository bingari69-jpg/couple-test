...
async function createDB({maxPower=true}={}){
 const db=new PGlite();
 ...
 await db.exec(fs.readFileSync('supabase/migrations/20260915_alkkagi_spacing.sql','utf8'));
 if(maxPower)await db.exec(fs.readFileSync('supabase/migrations/20260915_alkkagi_max_power.sql','utf8'));
