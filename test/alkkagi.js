...
(async()=>{
 ...
 const farAngle=R.aim(initial.stones[0],initial.stones[8]);
 const pushed=R.shoot(initial,0,farAngle,94,false).state,knocked=R.shoot(initial,0,farAngle,95,false).state;
 assert(pushed.stones[8].alive,'non-MAX pushes the distant target without eliminating it');
 assert(Math.hypot(pushed.stones[8].x-initial.stones[8].x,pushed.stones[8].y-initial.stones[8].y)>.01,'non-MAX still reaches and pushes');
 assert(!knocked.stones[8].alive,'MAX can eliminate the same distant target');
 assert.deepEqual(knocked.stones,R.shoot(initial,0,farAngle,100,false).state.stones,'entire MAX zone has the same strength');
 ...
 assert.deepEqual([0,650,1300,1950,2600].map(t=>R.gaugePower(t/1.5)),[10,55,100,55,10]);
 const frontGap=Math.min(...initial.stones.filter(s=>s.color===1).map(s=>s.y))-Math.max(...initial.stones.filter(s=>s.color===2).map(s=>s.y));
 assert(Math.abs(frontGap-.44)<1e-6,'opponent front rows are further apart');
 const attack=R.choose(initial);const attacked=R.shoot(initial,attack.id,attack.angle,attack.power,false).state;assert(attacked.stones.slice(5).some((s,i)=>s.x!==initial.stones[i+5].x||s.y!==initial.stones[i+5].y||!s.alive),'opponent remains reachable');
 ...
 s=await rpc(1,'join_alkkagi_room',{p_code:code,p_name:'친구'});assert.equal(s.me,2);assert.equal(s.physicsVersion,2);
 ...
 const preserved=s.board;await db.exec(require('node:fs').readFileSync('supabase/migrations/20260915_alkkagi_spacing.sql','utf8'));assert.deepEqual((await rpc(0,'get_alkkagi_room',{p_code:code})).board,preserved);
 ...
 // Upgrading preserves an active old-physics room; mutual rematch opts into MAX.
 const older=await createDB({maxPower:false});try{
  let old=await older.rpc(0,'create_alkkagi_room',{p_name:'이전 방',p_request:randomUUID()});
  old=await older.rpc(1,'join_alkkagi_room',{p_code:old.code,p_name:'친구'});
  const migration=require('node:fs').readFileSync('supabase/migrations/20260915_alkkagi_max_power.sql','utf8');
  await older.db.exec(migration);await older.db.exec(migration);
  let upgraded=await older.rpc(0,'get_alkkagi_room',{p_code:old.code});assert.deepEqual(upgraded.board,old.board);assert.equal(upgraded.physicsVersion,1);
  const predicted=R.shoot({...initial,stones:old.board,physicsVersion:1},0,270,85,false).state;
  upgraded=await older.rpc(0,'play_alkkagi_shot',{p_code:old.code,p_stone:0,p_angle:270,p_power:85,p_version:upgraded.version});
  assert.equal(predicted.physicsVersion,1);
  upgraded.board.forEach((p,i)=>{assert.equal(p.alive,predicted.stones[i].alive);assert(Math.abs(p.x-predicted.stones[i].x)<.000002);assert(Math.abs(p.y-predicted.stones[i].y)<.000002);});
  upgraded=await older.rpc(0,'resign_alkkagi_room',{p_code:old.code,p_version:upgraded.version});
  await older.rpc(0,'rematch_alkkagi_room',{p_code:old.code,p_round:upgraded.round});
  upgraded=await older.rpc(1,'rematch_alkkagi_room',{p_code:old.code,p_round:upgraded.round});assert.equal(upgraded.physicsVersion,2);assert.equal(upgraded.turn,2);
  console.log('PASS MAX: distant push vs knockout, 95–100 zone, legacy migration and rematch');
 }finally{await older.db.close();}
 ...
 $('place').click();now=1300/1.5;$('place').click();assert.equal($('moveCount').textContent,'1번');assert($('place').disabled);assert.match($('turnStatus').textContent,/백돌/);assert.match($('shotNote').textContent,/힘 100/);
