...
(async function(){
  ...
  const brandDom=new JSDOM(read('index.html'));
  const brandLink=brandDom.window.document.querySelector('a.brand');
  assert(brandLink && (brandLink.getAttribute('aria-label')||brandLink.querySelector('img')?.alt||brandLink.textContent).includes('같이놀자'),'이미지 로고에도 읽을 수 있는 서비스 이름이 있어야 함');
  brandDom.window.close();
