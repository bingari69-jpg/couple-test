...
async function main(){
 ...
 const realtimeCards=[...hd.querySelectorAll('#catalogList > a')];assert.equal(realtimeCards.length,3);assert.deepEqual(realtimeCards.map(a=>a.getAttribute('href')).sort(),['t/alkkagi/?mode=online','t/omok/?mode=online','t/tapbattle/?mode=online']);assert.ok(realtimeCards.every(a=>a.querySelector('.catalog-tags').textContent==='실시간대전'));assert.equal(hd.getElementById('catalogCount').textContent,'실시간대전 · 3가지');
