...
async function main(){
 ...
 input('letterBody','가'.repeat(448)+'💌');assert.equal($('letterCount').textContent,'449/450');
 input('letterBody','가'.repeat(449)+'👩‍❤️‍👨');assert.equal($('letterCount').textContent,'450/450');assert.equal($('packLetter').disabled,false);
 input('letterBody','가 나\n다');assert.equal($('letterCount').textContent,'5/450');
 input('recipient','지민');input('sender','민수');input('letterBody',body);
 $('changePaper').click();$('templateGrid').querySelector('[data-template="winter"]').click();$('viewTemplate').click();$('useTemplate').click();assert.equal($('letterBody').value,body);assert.equal($('recipient').value,'지민');
 ...
 assert.equal(hd.getElementById('catalogList').children.length,33);assert.equal(hd.getElementById('all').hidden,false);assert.equal(hd.getElementById('allButton'),null);
 ...
 assert.deepEqual(['t/next-scene/','t/know-me/','t/lucky/'].filter(h=>catalogCards.some(a=>a.getAttribute('href')===h)).length,3,'남은 심리 3종이 홈 목록에 있어야 함');
 /* 내려놓은 것들: 페이지는 살아 있지만 홈 목록에는 없어야 한다 */
 ['t/react/','t/living/','t/repair/','t/group-room/'].forEach(h=>assert.equal(catalogCards.some(a=>a.getAttribute('href')===h),false,h+' 는 홈에서 내렸다'));
 ...
 for(const rel of ['연인','부부','친구','가족','전체']){
   ...
   if(rel==='전체')assert.equal(shown.length,33);else assert.ok(shown.every(a=>[...a.querySelectorAll('.catalog-tags span')].some(t=>t.textContent===rel)));
