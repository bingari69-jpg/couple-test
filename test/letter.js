...
async function main(){
 ...
 const kr=load('t/letter/index.html',hash({...p,n:'지민',f:'민수'}),0.25,'Mozilla/5.0 (Linux; Android 14; wv) KAKAOTALK/10.8.3 (INAPP)'),outside=kr.window.document.getElementById('replyLetter').getAttribute('href');assert.match(outside,/^intent:\/\/bingari69-jpg.github.io\/couple-test\/t\/letter\/\?reply=1/);assert.match(outside,/#Intent;scheme=https;/);assert.equal(kr.window.document.getElementById('reader').hidden,false);const replyQuery='?'+outside.split('?')[1].split('#Intent;')[0],reply=load('t/letter/index.html',replyQuery);reply.window.document.getElementById('viewTemplate').click();reply.window.document.getElementById('useTemplate').click();assert.equal(reply.window.document.getElementById('recipient').value,'민수');assert.equal(reply.window.document.getElementById('sender').value,'지민');reply.window.close();kr.window.close();
