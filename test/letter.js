...
async function main(){
 ...
 assert.equal(hd.getElementById('catalogList').children.length,30);assert.equal(hd.getElementById('all').hidden,false);assert.equal(hd.getElementById('allButton'),null);
 ...
 assert.equal(new Set(catalogCards.map(a=>a.getAttribute('href'))).size,30);
 assert.deepEqual(['t/know-me/'].filter(h=>catalogCards.some(a=>a.getAttribute('href')===h)).length,1,'남은 심리가 홈 목록에 있어야 함');
 ...
 for(const rel of ['연인','부부','친구','가족','전체']){
   ...
   if(rel==='전체')assert.equal(shown.length,30);else assert.ok(shown.every(a=>[...a.querySelectorAll('.catalog-tags span')].some(t=>t.textContent===rel)));
