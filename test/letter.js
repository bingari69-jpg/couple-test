...
async function main(){
 ...
 assert.deepEqual(['t/next-scene/','t/know-me/','t/lucky/','t/group-room/'].filter(h=>catalogCards.some(a=>a.getAttribute('href')===h)).length,4,'타로·마음동물·운세·단체방이 홈 목록에 있어야 함');
 ...
 for(const rel of ['연인','부부','친구','가족','전체']){
   ...
   if(rel==='전체')assert.equal(shown.length,37);else assert.ok(shown.every(a=>[...a.querySelectorAll('.catalog-tags span')].some(t=>t.textContent===rel)));
