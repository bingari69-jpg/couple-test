...
async function main(){
 ...
 let nativeShared;w.navigator.share=async data=>{nativeShared=data;};$('kakaoSend').click();await tick();assert.equal(nativeShared.url,url);assert.equal(nativeShared.title,'너에게 편지가 도착했어요');delete w.navigator.share;
 let sent,done;
