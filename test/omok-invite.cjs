...
(async()=>{
 const {db,rpc}=await createDB();const {window:w}=load('omok','?mode=online'),$=id=>w.document.getElementById(id);let shares=[],copied='',calls=[];
 try{
  ...
  assert.equal(calls.filter(n=>n==='create_omok_room').length,1);assert.equal(shares.length,0,'never send automatically');
  $('inviteKakao').click();await until(()=>shares.length===1);
  const invite=shares[0];assert.equal(invite.title,'오목 한판 할래?');assert.equal(invite.textOnly,true);assert.match(invite.url,/^https:\/\/noljago\.co\.kr\/t\/omok\/\?mode=online#room=[A-F0-9]{32}$/);
