...
async function check(w){
 ...
 assert.equal(message.objectType,'feed','all invitations use the same image feed');assert.equal(message.content.imageHeight,480);assert.ok(message.content.imageUrl.includes('/assets/share-cards/'));assert.equal(message.content.link.webUrl,expected.url);assert.equal(message.content.link.mobileWebUrl,expected.url);assert.equal(message.buttonTitle,expected.btn);assert.equal(el(w,'sharePsy').disabled,true,'wait for SDK dispatch before re-enabling');
...
(async()=>{
 ...
 opened.forEach(w=>w.close());assert.deepEqual(PAGE_ERRORS,[]);console.log('실제 공유 버튼 검사 통과: 타로 봉인/결과·마음동물·운세 → 통일된 이미지 SDK 메시지, 대기 상태, 오류 시 정확한 링크 복사');
