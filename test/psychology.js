...
(async()=>{
 const hub=open('psychology');assert.equal(el(hub,'tests').children.length,4);
 const homeSwitch=[...hub.document.querySelectorAll('.home-switch a')];assert.deepEqual(homeSwitch.map(a=>a.textContent),['둘이놀기','혼자놀기','편지','심리']);assert.equal(homeSwitch[0].getAttribute('href'),'../../?home=play');assert.equal(homeSwitch[1].getAttribute('href'),'../../?tab=solo#all');assert.equal(homeSwitch[2].getAttribute('href'),'../letter/');assert.equal(homeSwitch[3].getAttribute('aria-current'),'page');assert.equal(el(hub,'menu').hidden,true);el(hub,'menuButton').click();assert.equal(el(hub,'menu').hidden,false);assert.equal(el(hub,'menuButton').getAttribute('aria-expanded'),'true');el(hub,'menuButton').click();assert.equal(el(hub,'menu').hidden,true);
 assert.equal(hub.document.querySelector('[data-category="운세"]').hidden,true,'운세: 항목 없는 분류는 숨긴다');
 for(const [cat,count] of [['성격',2],['관계',2],['전체',4]]){const btn=hub.document.querySelector('[data-category="'+cat+'"]');assert.equal(btn.hidden,false);btn.click();assert.equal(el(hub,'tests').children.length,count);}
 ...
 for(const [slug,hash] of [['personality','#r=1-99'],['personality','#r=<img>'],['fortune','#r=1-2026-02-30-0'],['fortune','#r=1-2026-09-10-99']])assert.equal(el(open(slug,hash),'invalid').hidden,false);
