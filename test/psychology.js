...
(async()=>{
 const hub=open('psychology');assert.equal(el(hub,'tests').children.length,5);
 ...
 for(const [cat,count] of [['성격',1],['관계',2],['운세',2],['전체',5]]){hub.document.querySelector('[data-category="'+cat+'"]').click();assert.equal(el(hub,'tests').children.length,count);}
