...
complete(guest,answers);assert.ok(shown(guest,'result'));assert.equal(el(guest,'score').textContent,'5 / 5');
assert.equal(el(guest,'scoreNote').textContent,'5문제 모두 맞혔어!');
assert.equal(guest.document.querySelectorAll('.compare-row.match').length,5);
const reply=grabKakao(guest,'sendResult');assert.ok(reply.url.includes('#r='));
const result=open(reply.url.slice(reply.url.indexOf('#')));assert.ok(shown(result,'result'));assert.equal(el(result,'score').textContent,'5 / 5');
...
const wrong=open(hash);el(wrong,'guessStart').click();complete(wrong,[[1],[2],[0],[2],[1]]);
assert.equal(el(wrong,'score').textContent,'0 / 5');assert.equal(wrong.document.querySelectorAll('.comparison').length,5);
assert.ok(el(wrong,'comparisons').textContent.includes('이건 의외지?'));
const partial=open(hash);el(partial,'guessStart').click();complete(partial,[[0],[1],[0],[1],[1]]);assert.equal(el(partial,'score').textContent,'3 / 5');
/* 예전 3문제 링크도 그대로 읽힌다 */
const legacy=open('#c='+b64e(JSON.stringify({v:2,p:1,n:'옛친구',a:[[0],[1],[2]]})));assert.ok(shown(legacy,'invite'));assert.ok(el(legacy,'invitePack').textContent.includes('3문제'));el(legacy,'guessStart').click();complete(legacy,[[0],[1],[2]]);assert.equal(el(legacy,'score').textContent,'3 / 3');
await Promise.resolve();
el(maker,'edit').click();choose(maker,[2]);el(maker,'next').click();el(maker,'next').click();el(maker,'next').click();el(maker,'next').click();el(maker,'next').click();
