...
(async () => {
  ...
  start(w);
  assert.equal(st.running, true); assert.deepEqual(food(w), f0); assert.equal(w.__ev('tickMs()'), 182);
  ...
  assert.equal(w.__ev('tickMs()'), 178, '한 마리에 4ms 빨라짐');
  ...
  assert.equal(w.__ev('state.eaten=30; tickMs()'), 116, '최소 116ms'); w.__ev('state.eaten=2');
  ...
  assert.equal(s.__ev('baseTick(1)'), 206); assert.equal(s.__ev('baseTick(5)'), 128); assert.equal(s.__ev('baseTick(0)'), 182);
  assert.equal(el(s, 'nameIn').classList.contains('hidden'), true); assert.equal(el(s, 'playTag').textContent, '혼자놀기');
  assert.match(el(s, 'soloDesc').textContent, /클리어 8마리/); assert.match(el(s, 'soloDesc').textContent, /★★★ 14마리/);
  const L = s.Solo.levels[0];
  assert.equal(s.Solo.starsFor(L, 14), 3); assert.equal(s.Solo.starsFor(L, 11), 2); assert.equal(s.Solo.starsFor(L, 8), 1); assert.equal(s.Solo.starsFor(L, 7), 0);
  const seedLv1 = ss.seed;

  /* 클리어: 14마리 먹고(중간에 한 번 부딪혀도) 시간 끝 → ★★★, 저장, Lv2 해제, 도전장 링크. 봉인 카드는 없음 */
  start(s);
  for (let i = 0; i < 7; i++) eatOne(s);
  crash(s); assert.equal(ss.done, false, '부딪혀도 안 끝남');
  for (let i = 0; i < 7; i++) eatOne(s);
  timeUp(s);
  const res = el(s, 'soloResult'); assert.ok(res, '혼자 결과 카드');
  assert.equal(res.querySelector('.stars').textContent, '★★★'); assert.match(res.querySelector('.rec').textContent, /14마리/); assert.match(res.querySelector('.rec').textContent, /부딪힘 1번/);
  ...
  assert.equal(saved.snake['1'].stars, 3); assert.equal(saved.snake['1'].best, 14);
  ...
  /* 화살표 버튼은 없고, 판을 톡 누르면 머리 기준으로 방향이 바뀐다 */
  {
    ...
    assert.ok(t.document.getElementById('dpad'), '화살표 버튼');
    ...
    t.__ev('state.pending="R"; state.dir="R"');
    t.document.querySelector('#dpad .u').dispatchEvent(new t.Event('pointerdown',{bubbles:true}));
    assert.equal(t.__ev('state.pending'), 'U', '화살표 버튼으로도 방향 전환');
    t.close();
  }
  ...
  assert.equal(el(s, 'speed').textContent, '한 칸 0.18초');
  ...
  assert.equal(ds.seed, seedLv1); assert.equal(ds.level, 1); assert.equal(el(du, 'speed').textContent, '한 칸 0.21초');
